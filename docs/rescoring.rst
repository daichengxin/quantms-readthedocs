...
The core functionality of quantms-rescoring lies in its intelligent model selection capabilities - it automatically evaluates and selects optimal MS2PIP and AlphaPeptDeep models based on fragmentation type and correlation quality, dynamically adjusts MS2 tolerance parameters, and employs sophisticated validation mechanisms to ensure model appropriateness for each dataset. Furthermore, quantms-rescoring supports AlphaPeptDeep transfer learning to generate project-specific model and compare it with the pretrained model, finally using the best model based on similarity metrics. This is particularly useful for PTM analysis. Additionally, it implements automatic benchmarking between pretrained and retrained DeepLC models, selecting the configuration with the lowest Mean Absolute Error for retention time prediction while performing per-run calibration to account for chromatographic variations.
...
The quantms-rescoring framework employs a sophisticated multi-tier methodology for feature generation, model selection, transfer learning and optimization that goes far beyond traditional rescoring approaches. **Feature generators are selected** through a configurable pipeline system where users can specify combinations of "ms2pip", "deeplc", "alphapeptdeep", and spectrum quality modules, with each generator contributing complementary information about spectral prediction accuracy, retention time behavior, and data quality.
...
- **AlphaPeptDeep transfer learning**: For AlphaPeptDeep, quantms supports transfer learning for given idXML identification results and mzML spectra data. quantms-rescoring then compared retrained model with pretrained model, and using the best model based on similarity metrics to avoid incorrect transfer learning. Leverages fine-tuning to adapt models to specific experimental/project conditions based on identifications file (idXML) from quantms, improving prediction accuracy for challenging datasets, such as PTM.
...
- **Transfer Learning**: Automatically performing transfer learning for given quantms identification results, and compared retrained model with pretrained model. Finally, using the best model based on similarity metrics to avoid incorrect transfer learning.
- **Intelligent Model Selection**: Competes with MS2PIP to select the best-performing model
- **Feature-Rich Output**: Generates comprehensive spectral similarity features

We implement same features as MS2PIP for AlphaPeptDeep (70+ features) to unify all rescoring features
...
The package implements multiprocessing capabilities for efficient handling of large datasets and maintains compatibility with OpenMS formats (idXML and mzML). It supports various fragmentation methods and MS analyzers through multiple pre-trained models and transfer learning.
...
- ``--ms2features_enable`` (default: ``false``): Enable/disable peptide identification rescoring with LC-MS predictors such as MS²PIP, AlphaPeptDeep and DeepLC
- ``--ms2features_range`` (default: ``independent_run``): Defines the scope of rescoring:
...
- ``--ms2features_model`` (default: ``HCD2021``): Specifies which deep learning model to use for feature generation
- ``--ms2features_model_dir`` (default: ``null``): Path to local MS2 prediction model files to avoid repeated downloads
- ``--ms2features_best`` (default: ``true``): Automatically find the best MS2 model for the dataset
- ``--ms2features_force`` (default: ``false``): Force usage of the specified MS2PIP model without optimization
- ``--ms2features_fine_tuning`` (default: ``false``): Whether to fine tune AlphaPeptDeep for identification rescoring
...
- ``--ms2features_calibration`` (default: ``0.15``): Percentage of data used as calibration set for DeepLC (15%)
- ``--ms2features_tolerance`` (default: ``0.05``): Fragment mass tolerance used for MS2 intensity features
- ``--ms2features_tolerance_unit`` (default: ``Da``): Unit for ms2features_tolerance (Da or ppm)

.. important:: For optimal results, ensure that your experimental data matches the properties of the selected MS2PIP and AlphaPeptDeep model in terms of fragmentation method, mass analyzer, and peptide modifications. We recommend to use the parameter ``--ms2features_best true`` and ``--ms2features_force false`` which will automatically select the best model for your dataset.
...
.. _supported-AlphaPeptDeep-models-table:
...
| Model        | Fragmentation method | MS2 mass analyzer                      | Peptide properties                                 |
+==============+======================+========================================+====================================================+
| generic      | HCD                  | Not Ion mobility                       | label free                                         |
+--------------+----------------------+----------------------------------------+----------------------------------------------------+
...
.. code-block:: bash
   ...
   nextflow run nf-core/quantms \
     --ms2features_enable true \
     --ms2features_model_dir /path/to/models \
     --ms2features_force true \
     --ms2features_generators deeplc,ms2pip \
     --input samplesheet.csv \
     --fasta database.fasta

**Rescoring with MS2 transfer learning:**
...
.. code-block:: bash
   ...
   nextflow run nf-core/quantms \
     ...
     --ms2features_fine_tuning true \
     --fine_tuning_sample_run 1 \
     --input samplesheet.csv \
     --fasta database.fasta
...
**Feature generator failures**: These might result from incorrect model parameters or unsupported experimental types leading to poor model predictions. In such cases, quantms-rescoring can be disabled using ``--ms2features_enable false`` (default setting).

**Memory and performance**: For large datasets, consider:

- Using ``--ms2features_range independent_run`` for better memory management
- Setting ``--ms2features_model_dir`` to a local directory to avoid repeated downloads
