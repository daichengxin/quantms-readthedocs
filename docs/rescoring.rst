...
- **AlphaPeptDeep transfer learning**: For AlphaPeptDeep, quantms support transfer learning for given idXML identification results and mzML spectra data. quantms-rescoring then compared retrained model with pretrained model, and using the best model based on similarity metrics to avoid incorrect transfer learning. Leverages fine-tuning to adapt models to specific experimental/project conditions based on identifications file (idXML) from quantms, improving prediction accuracy for challenging datasets, such as PTM.
